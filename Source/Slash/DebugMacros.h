...
#define DRAW_SPHERE(Location) \
    if (GetWorld())           \
    DrawDebugSphere(GetWorld(), Location, 25.f, 12, FColor::Blue, true)

#define DRAW_SPHERE_SingleFrame(Location) \
    if (GetWorld())                       \
    DrawDebugSphere(GetWorld(), Location, 25.f, 12, FColor::Blue, false, -1.f)

#define DRAW_LINE(StartLocation, EndLocation) \
    if (GetWorld())                           \
    DrawDebugLine(GetWorld(), StartLocation, EndLocation, FColor::Yellow, true, -1, 0, 1.f)

#define DRAW_LINE_SingleFrame(StartLocation, EndLocation) \
    if (GetWorld())                                       \
    DrawDebugLine(GetWorld(), StartLocation, EndLocation, FColor::Yellow, false, -1, 0, 1.f)
